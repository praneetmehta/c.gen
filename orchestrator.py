...
from content_provider.config.content_provider_request_factory import ContentProviderRequestFactory
...
class Orchestrator:
    ...
    def run(self, request : OrchestrationRequest):
        ...
        if (content_provider != None):
            content : Frame = content_provider.generate_content(ContentProviderRequestFactory.get_request(request.content_provider_type),
                                                                ContentProviderRequestFactory.get_request_config(request.content_provider_type))
            if content.use_segmented and content.content_segmented != None:
                for i, segment in tqdm(enumerate(content.content_segmented)):
                    generate_image(Request(segment, dir_to_use + '/frames/{}.png'.format(i)))

        os.system("./ffmpeg -framerate 0.2 -i " + dir_to_use + "/frames/%1d.png -vf fps=15 -pix_fmt yuv420p " + dir_to_use +"/output_without_audio.mp4")
...
if __name__ == "__main__":
    ...
    orch.run(OrchestrationRequest(request_topic="amsterdam", run_id=run_id, image_config=img_config, text_config=txt_config, content_provider_type=ContentProviderType.TCT))